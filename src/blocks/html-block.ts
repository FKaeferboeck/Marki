...
export const htmlBlock_traits: BlockTraits<"htmlBlock"> = {
    startsHere(LLD, B, interrupting) {
        ...
        if(/^<(pre|script|style|textarea)(?=[\s\t>]|$)/i.test(LLD.startPart)) {
            B.htmlType   = "preEtAl";
            B.singleLine = (this.traits.continuesHere!.call(this, LLD) === "last");
            return 0;
        }

        if(/^<!--/.test(LLD.startPart)) {
            B.htmlType   = "comment";
            B.singleLine = (this.traits.continuesHere!.call(this, LLD) === "last");
            return 0;
        }
        ...
        if(/^<![A-Za-z]/.test(LLD.startPart)) {
            B.htmlType   = "definition";
            B.singleLine = (this.traits.continuesHere!.call(this, LLD) === "last");
            return 0;
        }

        if(/^<!\[CDATA\[/.test(LLD.startPart)) {
            B.htmlType   = "CDATA";
            B.singleLine = (this.traits.continuesHere!.call(this, LLD) === "last");
            return 0;
        }
        ...
        if((rexres = /<\/?([A-Za-z]+)(?=[ \t>]|\/>|$)/.exec(LLD.startPart)) && case6elements[rexres[1].toLowerCase()]) {
        ...
        if(interrupting)
            return -1;
        if(singleLineOpeningTag.test(LLD.startPart) || singleLineClosingTag.test(LLD.startPart)) {
    ...
    continuesHere(LLD) {
        ...
        case "preEtAl":
            return (/<\/(?:pre|script|style|textarea)>/i.test(LLD.startPart) ? "last" : 0);
        case "comment":
            return (/-->/.test(LLD.startPart) ? "last" : 0);
        ...
        case "knownElement":
        case "anyTag":
            return (LLD.type === "empty" || LLD.type === "emptyish" ? "end" : 0);
        default:
